...
tf.set_random_seed(1)  # 这两行语句暂时不管
np.random.seed(1)

# Hyper parameters
N_SAMPLES = 2000  # 设置train数据的多少
BATCH_SIZE = 64
EPOCH = 12
LR = 0.03
N_HIDDEN = 8
...
# training data
x = np.linspace(-7, 10, N_SAMPLES)[:, np.newaxis]  # 产生一个(2000 *　1) 类型的数据
np.random.shuffle(x)  # 将x随机打乱,降低数据之间产生相关性
noise = np.random.normal(0, 2, x.shape)  # 产生和x一样的noise数据 (2000 * 1)
y = np.square(x) - 5 + noise  # 得到输出数据y　
train_data = np.hstack((x, y))   # 将x,y进行合成,生成一个 (2000 * 2) 类型的数据

# test data

test_x = np.linspace(-7, 10, 200)[:, np.newaxis]  # 测试数据(200 * 1)
noise = np.random.normal(0, 2, test_x.shape)
test_y = np.square(test_x) - 5 + noise  # 测试数据 (200 * 1)

# plot input data
plt.scatter(x, y, c='#FF9359', s=50, alpha=0.5, label='train')   # 先绘制出输入的train数据
plt.legend(loc='upper left')
# print("train data")  # 测试语句
# tensorflow placeholder
tf_x = tf.placeholder(tf.float32, [None, 1])  # 设置tensorflow一批训练数据
tf_y = tf.placeholder(tf.float32, [None, 1])   # 设置tensorflow一批训练数据
tf_is_train = tf.placeholder(tf.bool, None)     # flag for using BN on training or testing 待定？


class NN(object):
    def __init__(self, batch_normalization=False):  # 默认不进行BN
